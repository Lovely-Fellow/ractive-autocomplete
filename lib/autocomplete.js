...
var template
	...
	+	'<input value="{{_state.text}}" on-blur="blur" on-click="show" on-keypress="keydown" {{#if placeholder}}placeholder="{{placeholder}}"{{/if}}>'
	+		'{{#if _state.visible}}'
	+		'<ul on-click="click">{{#_state.suggestions:i}}'
	+			'<li {{#if _state.isSelected(.)}}class="selected"{{/if}}>{{_state.getText(.)}}</li>'
	+		'{{/_state.suggestions}}</ul>'
	+		'{{/if}}'
	+ '</div>';
